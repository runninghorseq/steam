...
function parseSteamAccounts(filePath) {
    try {
        ...
        const accounts = lines.map((line, index) => {
            if (line.includes('----')) {
                // Format: username----password----email----steamID----emailServer[|emailSteamID]
...
const run = ['pittafridyc', 'nookstostazr']
...
// Set FORCE=true (or pass --force) to ignore the processed-lines filter and rerun from the top
...
const processedRawLines = FORCE ? new Set() : readProcessedRawLines(FILE_NAME);
const remaining = allAccounts.filter(a => !processedRawLines.has(a.rawLine));
const accounts = remaining.slice(0, BATCH_SIZE);
console.log(`Force rerun: ${FORCE} | Already processed: ${processedRawLines.size} | Remaining: ${remaining.length} | This batch: ${accounts.length}`);
...
clients.forEach((accClient, index) => {
    ...
    accClient.on('loggedOn', function() {
        ...
        if (personaName) {
            setTimeout(() => {
                accClient.setPersona(SteamUser.EPersonaState.Online, personaName);
                console.log(`[${account.id}] Set persona name to: ${personaName}`);
            }, 3000);
        }
